            ...
            while ant.energy > 0 and len(ant.visited_sensors) < len(self._list_of_sensors):
                ...
                if ant.energy < 0:
                    ...
                    if ant.energy < 0:
                        ...
                        ant.squares_visited_by_sensors[next_sensor] = \
                        self._list_of_sensors[next_sensor].discovered_squares[old_energy]
                        ant.energy = 0
                    else:
                        ant.squares_visited_by_sensors[next_sensor] = \
                        self._list_of_sensors[next_sensor].discovered_squares[energy_needed]
    ...
    def updateProbabilityRanges(self, ant):
        ...
        for j in range(1, len(self._list_of_sensors)):
            if j not in ant.visited_sensors:
                num.append(self._pheromone_matrix[current_sensor][j] ** alpha * self._visibility_matrix[current_sensor][
                    j] ** beta)
        ...
        for i in range(len(self._list_of_sensors)):
            for j in range(len(self._list_of_sensors)):
                self._sensors_paths[i].append(
                    self.searchAStar(self._repository.cmap, self._list_of_sensors[i].x, self._list_of_sensors[i].y,
                                     self._list_of_sensors[j].x, self._list_of_sensors[j].y))
    ...
    def searchAStar(self, mapM, initialX, initialY, finalX, finalY):
        ...
        while len(open) > 0:
            ...
            for i in range(0, 4):
                ...
                if new_pos not in closed and new_pos[0] >= 0 and new_pos[0] < mapM.n and new_pos[1] >= 0 and new_pos[
                    1] < mapM.m:
                    if mapM.surface[new_pos[0]][new_pos[1]] == 0 or mapM.surface[new_pos[0]][new_pos[1]] == 2:
                        ...
                        totalDistance[new_pos[0]][new_pos[1]] = startDistance[new_pos[0]][new_pos[1]] + \
                                                                goalDistance[new_pos[0]][new_pos[1]]
